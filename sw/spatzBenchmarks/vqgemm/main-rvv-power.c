...
int main() {
  const unsigned int PWR_M = 16u; // output rows (codebooks and K unchanged)
  ...
  if (cid == 0) {
    ...
    if (vq_l.CB_D == 8) {
      vqgemm_rvv_d8(vq_c, vq_a, vq_cb0, vq_cb1, (const uint8_t *)vq_idx0,
                    (const uint8_t *)vq_idx1, vq_scales, vq_wrow, PWR_M,
                    vq_l.N, vq_l.K);
    } else {
      vqgemm_rvv_d16(vq_c, vq_a, vq_cb0, vq_cb1, (const uint16_t *)vq_idx0,
                     (const uint16_t *)vq_idx1, vq_scales, vq_wrow, PWR_M,
                     vq_l.N, vq_l.K);
    }
