...
int main() {
  const unsigned int PWR_M = 16u; // output rows = 2 blocks of 8 (codebooks and K unchanged)
  ...
  if (cid == 0) {
    ...
    // error = verify_output(vq_c, vq_expected, PWR_M, vq_l.N);

#ifdef PRINT_RESULT
    printf("vqgemm vlxblk M=%u N=%u K=%u CB_D=%u CBN=%u cache=%d: took %u "
           "cycles %s (macs=%u)\n",
           PWR_M, vq_l.N, vq_l.K, vq_l.CB_D, vq_l.CBN, USE_CACHE, timer,
           error ? "CHECK-FAILED" : "CHECK-OK", PWR_M * vq_l.N * vq_l.K);
#endif
  }
