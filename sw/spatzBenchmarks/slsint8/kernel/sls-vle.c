...
// slsint8, vle baseline: the same dequant pipeline as the VLXBLK arm, the
// row loaded with a scalar-addressed unit-stride vector load (t + id *
// row_d) instead of an indexed block gather. Scale/bias pairs are
// scalar-loaded via the same index (flh + fcvt.s.h) — the dequant work is
...
void sls_vle(float *out, const uint8_t *t, const __fp16 *sb,
  ...
  for (unsigned int b = 0; b < nb; ++b) {
    ...
    for (unsigned int l = 0; l < lp; ++l) {
      ...
      asm volatile("vle16.v v4, (%0)" ::"r"(row) : "memory");
