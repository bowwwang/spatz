...
// slsint8, vle baseline arm. Identical data path to main-vlxblk.c; only
// the kernel differs.
...
int main() {
  ...
  if (cid == 0) {
    ...
    sls_vle(sl_out, sl_tbl, sl_sb, sl_idx, sl_l.NB, sl_l.LP, sl_l.ROW_D);
...
#ifdef PRINT_RESULT
    printf("slsint8 vle nb=%u lp=%u row_d=%u nrows=%u cache=%d: took %u "
           "cycles %s\n",
