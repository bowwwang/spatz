...
// attn-vagg, plain-RVV (vle) baseline arm. Identical data path to
// main-vlxblk.c; only the kernel differs.
...
int main() {
  ...
  if (cid == 0) {
    ...
    attn_vle(attn_out, attn_pool, attn_idx, attn_p, attn_l.NQ, attn_l.TOPK,
             attn_l.HD);
