...
int main() {
  const unsigned int PWR_NB = 32u; // bags (table stays 2 MiB, 40 lookups per bag)
  ...
  if (cid == 0) {
    ...
    sls32_vle(sl_out, (const float *)sl_tbl_bits, sl_idx, PWR_NB, sl_l.LP,
              sl_l.ROW_D);
    ...
    // error = verify_output(sl_out, sl_expected, PWR_NB, sl_l.OUT_D);

#ifdef PRINT_RESULT
    printf("sls32 vle nb=%u lp=%u row_d=%u nrows=%u cache=%d: took %u "
           "cycles %s\n",
           PWR_NB, sl_l.LP, sl_l.ROW_D, sl_l.NROWS, USE_CACHE, timer,
