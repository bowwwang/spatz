...
// slsint8, vle arm. ALL data (table, scale/bias, indices, expected output)
...
int main() {
  ...
  if (cid == 0) {
    ...
    sls_vle(sl_out, (const __fp16 *)sl_tbl_bits, sl_idx, sl_l.NB, sl_l.LP, sl_l.ROW_D, sl_l.DBG_EVERY);
...
#ifdef PRINT_RESULT
    printf("sls vle nb=%u lp=%u row_d=%u nrows=%u cache=%d: took %u "
           "cycles %s\n",
