...
// pr-gather, VLXBLK arm. Sizes, constants, hash seed and the expected
// output come from the generated DATAHEADER; the contribution table and
// the neighbor-id list are generated on-core from the closed-form
// patterns the generator mirrors bit-exactly (see script/gen_data.py).
...
// gen_data.py. vfredosum's internal association differs from the
// sequential C sum in final ULPs on this implementation (v=4 diagnostic,
// 2026-09-08: got==exp to 6 decimals) -> 1e-9 relative tolerance; a
// single wrong index perturbs the sum by >= ~6e-5 relative, four orders
// above this bound, so index errors stay fully detectable.
...
int main() {
  ...
  if (cid == 0) {
    ...
    pr_vlxblk(pr_out, pr_contrib, pr_nbr, pr_l.NACT, pr_l.DEG, pr_base,
              pr_damp);
...
#ifdef PRINT_RESULT
    printf("pr-gather vlxblk nv=%u nact=%u deg=%u cache=%d: took %u cycles "
           "%s (adds=%u)\n",
