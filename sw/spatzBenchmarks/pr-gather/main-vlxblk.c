...
// pr-gather, VLXBLK arm. All data (contribution table, neighbor ids,
// reduction seed, expected output) is literal in the generated DATAHEADER
...
// (fp64 bit patterns read with integer loads: no FPU loads after the
// kernel's vector traffic). 2^20 ULPs ~ 2.3e-10 relative absorbs
// vfredosum's internal association; one wrong index perturbs a sum by
// >= ~1e-2 relative.
...
int main() {
  ...
  if (cid == 0) {
    ...
    pr_vlxblk(pr_out, (const double *)pr_contrib_bits, pr_nbr, pr_seed,
              pr_l.NACT, pr_damp);
