...
int main() {
  const unsigned int PWR_NB = 16u; // nodes (table stays 4 MiB)
  ...
  if (cid == 0) {
    ...
    agg_vle_d64(ga_out, (const float *)ga_tbl_bits, ga_idx_rows, PWR_NB,
                ga_l.LP);
    ...
    // error = verify_output(ga_out, ga_expected_bits, PWR_NB * ga_l.ROW_D,
    //                           ga_l.ROW_D);
