...
// gatheragg, VLXBLK arm. Data (transposed indices, per-bag checksums) from
// the generated DATAHEADER; the embedding table (2 / 4 MiB) filled on-core
// from the closed-form pattern the generator mirrors (see
// script/gen_data.py).
...
int main() {
  ...
  if (cid == 0) {
    ...
    agg_vlxblk(ga_out, ga_tbl, ga_idx, ga_l.NB, ga_l.ROW_D, ga_l.LP,
               ga_l.DBG_EVERY);
